...
TOOL_SPEC_ROWS = {  # fmt: skip
    "shell_command": ("run_shell_command", True, ("run", "check", "read", "list"), "Run a shell command using the user's shell. Commands run in the project by default; pass cwd only when the user explicitly named another directory. Shell syntax such as &&, ;, pipes, redirects, environment assignments, and globbing is allowed. The tool itself asks for confirmation when required, so do not ask separately in assistant text. On macOS and Linux, invoke Python scripts with python3 rather than python. Use timeout_s for long builds, installs, updates, or tests. Do not run malicious or unrelated destructive commands.", {"command": {"type": "string"}, "cwd": {"type": "string", "description": "Optional working directory. Use an explicit absolute path when the user asked to run the command outside the project."}, "timeout_s": {"type": "integer", "description": "Maximum runtime in seconds. Defaults to 600; capped at 7200."}}, ("command",), False),
}
