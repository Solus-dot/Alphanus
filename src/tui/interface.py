...
class AlphanusTUI(App):
    ...
    def _save_active_session(self, rename_to: str | None = None) -> ChatSession:
    ...
    def _on_session_manager_close(self, result: dict[str, str] | None) -> None:
        on_tui_session_manager_close(self, result)

    def _on_session_name_close(self, result: dict[str, str] | None) -> None:
    ...
    @staticmethod
    def _bar_renderable(renderable: RenderableType, color: str, *, content_indent: int = 0, continuation_indent: int | None = None):
        return tui_bar_renderable(renderable, color, content_indent=content_indent, continuation_indent=continuation_indent)
    ...
    def _syntax_renderable(self, code: str, language: str | None):
        return tui_syntax_renderable(
            self,
            code,
            language,
            syntax_theme=self._theme_spec().syntax_theme,
            background_color=self._theme_color("panel_bg", DEFAULT_PANEL_BG),
        )

    def _code_panel_renderable(self, code: str, language: str | None):
    ...
    def _tool_event_panel(self, title: str, title_color: str, border_color: str, name: str, detail: str = ""):
    ...
    def _remember_code_block(self, code: str, language: str | None) -> int:
        return tui_remember_code_block(self, code, language)

    def _write_code_block(self, lines: list[str], language: str | None, content_indent: int = 2) -> None:
    ...
    def _update_live_preview_partial(self, lines: list[str], language: str | None) -> None:
        tui_update_live_preview_partial(self, lines, language)

    def _defer_live_preview_partial(self, lines: list[str], language: str | None) -> None:
    ...
    def _capture_scroll_anchor(self) -> ScrollAnchor | None:
        return tui_capture_scroll_anchor(self)

    def _restore_scroll_anchor(self, anchor: ScrollAnchor | None) -> None:
    ...
    def _write_section_heading(self, title: str, color: str | None = None) -> None:
        tui_write_section_heading(self, title, color=color or self._theme_color("accent", DEFAULT_ACCENT_COLOR))

    def _write_detail_line(self, label: str, value: str, *, value_markup: bool = False) -> None:
        tui_write_detail_line(self, label, value, accent_color=self._theme_color("accent", DEFAULT_ACCENT_COLOR), value_markup=value_markup)

    def _write_indexed_dim_lines(self, rows: list[str], *, color: str | None = None, allow_markup: bool = False) -> None:
        tui_write_indexed_dim_lines(self, rows, color=color or self._theme_color("accent", DEFAULT_ACCENT_COLOR), allow_markup=allow_markup)

    def _write_command_action(self, text: str, *, icon: str = "•", color: str | None = None) -> None:
        tui_write_command_action(self, text, color=color or self._theme_color("accent", DEFAULT_ACCENT_COLOR), icon=icon)
    ...
    def _reply_acc(self) -> str:
        return "".join(getattr(self, "_reply_acc_parts", []))
    ...
    @property
    def _fence_lang(self) -> str | None:
        return self._stream_runtime_state().text.fence_lang

    @_fence_lang.setter
    def _fence_lang(self, value: str | None) -> None:
    ...
    def _on_global_palette_close(self, result: dict[str, str] | None) -> None:
    ...
    def _on_theme_picker_close(self, result: dict[str, str] | None) -> None:
    ...
    def _on_config_editor_close(self, result: dict[str, Any] | None) -> None:
        on_tui_config_editor_close(self, result, config_path=_global_config_path(), status_runtime_state_cls=StatusRuntimeState)
