...
def cmd_skills(app: Any, *, accent_color: str) -> None:
    ...
    for skill in skills:
        ...
        if not skill.available and skill.availability_reason:
            ...
            app._write(
                f"    [bold {accent_color}]blocked ({code}):[/bold {accent_color}] [#a1a1aa]{esc(skill.availability_reason)}[/#a1a1aa]"
            )
...
def cmd_skill(app: Any, arg: str) -> bool:
    parts = arg.split()
    if not parts:
        return app._write_usage(
            "/skill-on <id> | /skill-off <id> | /skill-unload <id> | /skill-unload-all | /skill-reload | /skill-info <id>"
        )
...
def cmd_doctor(app: Any, *, accent_color: str) -> None:
    ...
    for skill in report.get("skills", []):
        app._write(
            f"  [bold {accent_color}]{esc(str(skill.get('id', '')))}[/bold {accent_color}] [#a1a1aa]({esc(str(skill.get('provenance', '')))} · {esc(str(skill.get('availability_code', 'ready')))})[/#a1a1aa] [#a1a1aa]{esc(str(skill.get('status', 'unknown')))}[/#a1a1aa]"
        )
