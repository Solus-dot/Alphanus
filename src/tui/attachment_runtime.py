...
def workspace_root(app: Any) -> Path: return Path(str(app.agent.skill_runtime.workspace.workspace_root)).resolve()
def home_root(app: Any) -> Path: candidate = getattr(app.agent.skill_runtime.workspace, "home_root", None); return Path(str(candidate)).resolve() if candidate else Path.home().resolve()
def attachment_root_path(app: Any, root_id: str) -> Path: return home_root(app) if root_id == "home" else workspace_root(app)
def attachment_root_label(root_id: str) -> str: return "Home" if root_id == "home" else "Workspace"
def root_relative_label(path: Path, root: Path) -> str:
    try:
        relative = path.resolve().relative_to(root); text = relative.as_posix()
...
def attach_file_path(app: Any, path: str | Path) -> bool:
    ...
    if not resolved.is_file():
        app._write_error(f"File not found: {resolved}"); return False
    kind = classify_attachment(str(resolved))
    if kind == "unknown":
        app._write_error("Unsupported file type"); return False
    normalized = str(resolved)
    if any(existing_path == normalized for existing_path, _existing_kind in app.pending):
        app._write_info(f"Already attached: {resolved.name}"); return False
    app.pending.append((normalized, kind))
    app._update_pending_attachments(); app._update_status1(); return True
def attachment_picker_items(app: Any, relative_dir: str = ".", *, root_id: str = "workspace", accent_color: str = DEFAULT_ACCENT_COLOR) -> List[PickerItem]:
    ...
    for candidate_root in ("workspace", "home"):
        if candidate_root == root_id:
            continue
        items.append(PickerItem(id=f"root:{candidate_root}:.", prompt=f"[bold {accent_color}]switch -> {attachment_root_label(candidate_root).lower()}[/bold {accent_color}]"))
    ...
    for entry in entries:
        ...
        if entry.endswith("/"):
            items.append(PickerItem(id=f"nav:{target}", prompt=f"[bold {accent_color}]{esc(entry)}[/bold {accent_color}] [dim]open[/dim]"))
            continue
        target_path = root_path / target
        kind = classify_attachment(str(target_path))
        if kind == "unknown":
            continue
        items.append(PickerItem(id=f"file:{target}", prompt=f"[{text_color}]{esc(entry)}[/{text_color}] [dim]{kind}[/dim]"))
    return items
def open_attachment_picker(app: Any, relative_dir: str = ".", root_id: str = "workspace", accent_color: str = DEFAULT_ACCENT_COLOR) -> None:
    ...
    app.push_screen(SelectionPickerModal(title=title, subtitle=subtitle, confirm_label="Open / Attach", empty_text="No attachable files in this folder.", items=items), lambda result: on_attachment_picker_close(app, root_id, result, accent_color=accent_color))
def on_attachment_picker_close(app: Any, root_id: str, result: Optional[Dict[str, str]], *, accent_color: str = DEFAULT_ACCENT_COLOR) -> None:
    selection = str((result or {}).get("id") or "").strip()
    if not selection:
        app.query_one("#chat-input").focus(); return
    if selection.startswith("root:"):
        _, next_root, next_dir = selection.split(":", 2)
        open_attachment_picker(app, next_dir or ".", root_id=next_root or "workspace", accent_color=accent_color); return
    if selection.startswith("nav:"):
        open_attachment_picker(app, selection[4:] or ".", root_id=root_id, accent_color=accent_color); return
