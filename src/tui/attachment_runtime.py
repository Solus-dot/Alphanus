...
def open_attachment_picker(app: Any, relative_dir: str = ".", root_id: str = "workspace", accent_color: str = DEFAULT_ACCENT_COLOR) -> None:
    ...
    app.push_screen(
        SelectionPickerModal(
            title=title, subtitle=subtitle, confirm_label="Open / Attach", empty_text="No attachable files in this folder.", items=items
        ),
        lambda result: on_attachment_picker_close(app, root_id, result, accent_color=accent_color),
    )


def on_attachment_picker_close(app: Any, root_id: str, result: dict[str, str] | None, *, accent_color: str = DEFAULT_ACCENT_COLOR) -> None:
