...
def handle_command(app, text: str) -> bool:
    ...
    if cmd == "/detach":
        ...
        if target == "all":
            ...
            app._update_pending_attachments()
            app._update_status1()
            app._write_command_action(f"Removed {count} attachment{'s' if count != 1 else ''}", icon="−")
            return True
        ...
        if target == "last":
            ...
            app._update_pending_attachments()
            app._update_status1()
            app._write_command_action(f"Removed attachment '{os.path.basename(removed_path)}'", icon="−")
            return True
