...
def trace_add(state: TurnState, key: str, row: dict[str, object]) -> None:
    rows = trace_list(state, key)
    rows.append(row)
    set_trace_list(state, key, rows)


def build(state: TurnState, result: AgentTurnResult, *, collaboration_mode: str) -> JsonObject:
    ...
    for item in trace_list(state, "passes"):
    ...
    return {
        ...
        "turn_trace": {
            "passes": cast(JSONValue, trace_list(state, "passes")),
            "tool_calls": cast(JSONValue, trace_list(state, "tool_calls")),
            "tool_results": cast(JSONValue, trace_list(state, "tool_results")),
        },
    }
