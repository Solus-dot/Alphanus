...
class FinalizationEngine:
    ...
    def finalize_response(
        ...
        *,
        ...
        def finalize(extra_rules: str = "") -> AgentTurnResult:
            return self.finalize_turn(system_content, state, stop_event, on_event, pass_id, extra_rules)

        def retry_once(flag: str, reason: str) -> bool:
            if getattr(state, flag):
                return False
            setattr(state, flag, True)
            self.orchestrator.emit(on_event, {"type": "discard_pass_output", "pass_id": pass_id, "reason": reason})
            return True
        ...
        if state.classification.time_sensitive and state.search_tools_enabled and state.completion.tool_counts.get("web_search", 0) == 0:
            if retry_once("forced_search_retry", "forced_search_retry"):
        ...
        if (
            ...
            if not final.strip():
                if retry_once("forced_action_retry", "forced_action_retry"):
                    return "continue", None
