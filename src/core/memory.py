...
class LexicalMemory:
    ...
    def __init__(self, storage_path: str, min_score: float = 0.3, persist_access_updates: bool = False,
        ...
        self.min_score = LegacyLexicalMemory._normalize_threshold(min_score, default=0.3)
        self.persist_access_updates = bool(persist_access_updates)
    ...
    def add_memory(self, text: str, memory_type: str = "conversation", metadata: dict[str, Any] | None = None,
        ...
        return LegacyLexicalMemory._to_public(MemoryItem(cursor.lastrowid or 0, str(text), md, str(memory_type), now, 0, now))

    def search(self, query: str, top_k: int = 5, memory_type: str | None = None,
               min_score: float | None = None) -> list[dict[str, Any]]:
        query_tokens = LegacyLexicalMemory._tokenize(query)
        if not query_tokens:
            return []
        threshold = self.min_score if min_score is None else LegacyLexicalMemory._normalize_threshold(min_score, default=self.min_score)
        ...
        scorer = LegacyLexicalMemory.__new__(LegacyLexicalMemory)
        scored: list[tuple[float, MemoryItem]] = []
        for row in rows:
            item = self._row_to_item(row)
            score = LegacyLexicalMemory._score_tokens(scorer, query_tokens, LegacyLexicalMemory._tokenize(item.text))
            if score >= threshold:
                scored.append((score, item))
        ...
        for score, item in selected:
            item.access_count += 1
            item.last_accessed = now
            record = LegacyLexicalMemory._to_public(item)
    ...
    def list_recent(self, count: int = 5) -> list[dict[str, Any]]:
        ...
        return [LegacyLexicalMemory._to_public(self._row_to_item(row)) for row in rows]
